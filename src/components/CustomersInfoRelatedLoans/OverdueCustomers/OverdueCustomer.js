...
class OverdueCustomerCardview extends Component {
  constructor(props) {
    super(props);
    this.state = {
      whiteimage:"none",
  ...
  closemodal=()=> {
    ...
    render(){
      ...
      const percentage = 66;
      let newData = this.props.overdueData.map(data => {
        let amount = 0;
        let totalAmount = 0;
        let percent = 0;
        ...
        return(
            ...
            <div>
                ...
                <Grid container spacing={1} style={{margin:"0px",marginRight: "43px"}}>
                   {this.props.overdueData && newData.map((open, index) =>{
      ...
      <CardHeader
        ...
        subheader= {
          ...
          <div className="subheaderStatus Fonts"  style={{backgroundColor:"#ffe5e5", color:"#FF0000"}}>{open.loanPaymentStatus}</div>
    ...
    :
            <OverdueCustomerListView/>
            }; 
            </div>         
        )
    }
}
export default withNamespaces()(withRouter(OverdueCustomerCardview));